...
struct RNDRDrawAnimatedSprites: RNDRDrawWorldPhase {
    ...
    func draw(world: GMWorld, encoder: MTLRenderCommandEncoder, camera: Float4x4) {
        ...
        renderables += world.monsters.map { monster in
            let billboard = RNDRBillboard.from(monster: monster, direction: world.directionParallelToPlayer)
            return (
                model.vertices,
                model.uv,
                worldTransform(billboard: billboard),
                GMColor.black,
        ...
        encoder.setFragmentTexture(renderer.spriteSheets[.monster]!, index: 0)
        encoder.setFragmentTexture(renderer.spriteSheets[.monsterWalk1]!, index: 1)
        encoder.setFragmentTexture(renderer.spriteSheets[.monsterWalk2]!, index: 2)
        encoder.setFragmentTexture(renderer.spriteSheets[.monsterScratch1]!, index: 3)
        encoder.setFragmentTexture(renderer.spriteSheets[.monsterScratch2]!, index: 4)
        encoder.setFragmentTexture(renderer.spriteSheets[.monsterScratch3]!, index: 5)
        encoder.setFragmentTexture(renderer.spriteSheets[.monsterScratch4]!, index: 6)
        encoder.setFragmentTexture(renderer.spriteSheets[.monsterScratch5]!, index: 7)
        encoder.setFragmentTexture(renderer.spriteSheets[.monsterScratch6]!, index: 8)
        encoder.setFragmentTexture(renderer.spriteSheets[.monsterScratch7]!, index: 9)
        encoder.setFragmentTexture(renderer.spriteSheets[.monsterScratch8]!, index: 10)
        encoder.setFragmentTexture(renderer.spriteSheets[.monsterHurt]!, index: 11)
    ...
    private func worldTransform(billboard: RNDRBillboard) -> Float4x4 {
        Float4x4.identity()
